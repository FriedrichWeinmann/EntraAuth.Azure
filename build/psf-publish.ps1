...
if ($LocalRepo) {
	Write-Host "Creating Nuget Package for module: 'EntraAuth.Azure' at '$(Get-Location)"
	Publish-PSFModule -Path "$($publishDir)\EntraAuth.Azure" -DestinationPath . -SkipDependenciesCheck
	return
}

# Publish to Gallery
Write-Host "Publishing the EntraAuth.Azure module to $($Repository)"
$param = @{
	Path                  = "$($publishDir)\EntraAuth.Azure"
